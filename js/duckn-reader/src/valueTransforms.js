 ...
 */
const IDENTITY = Object.freeze({ slope: 1, intercept: 0 });
/**
...
function applyLut(stored, values, firstValue) {
    if (stored instanceof Float32Array || stored instanceof Float64Array) {
        throw new Error("duckn: lut transform requires integer stored values, got a float " +
    ...
    for (let i = 0; i < stored.length; i++) {
        ...
        if (!Number.isInteger(v)) {
            throw new Error(`duckn: lut transform requires integer stored values, got ${v}`);
        }
...
export function planCalibration(transforms) {
    ...
    for (let i = 0; i < transforms.length; i++) {
        if (transforms[i].name === "lut" && i !== 0) {
            throw new Error(`duckn: value_transforms[${i}] is a 'lut': a lut indexes stored ` +
    ...
    for (const t of transforms) {
        if (t.name === "linear") {
            ...
            throw new Error(`duckn: unsupported value_transform '${t.name}': the value mapping ` +
