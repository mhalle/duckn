...
export interface Calibration {
  /** Affine part, safe to hand to a consumer that applies its own rescale. */
  slope: number;
  intercept: number;
  /**
   * Present when the chain could not be reduced to an affine mapping. The
   * caller must run this over the stored values and then treat
   * slope/intercept as the identity they are set to.
   */
 ...
 * this at all.
 */
export function planCalibration(transforms?: ValueTransform[] | null): Calibration {
  ...
  let slope = 1;
  let intercept = 0;
  let lut: { values: number[]; firstValue: number } | null = null;
