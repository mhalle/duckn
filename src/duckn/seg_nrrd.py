...
def _is_nrrd_background(seg: Segment) -> bool:
    ...
    return seg.role == "background" and seg.sorted_values == [0]
...
def needs_materialization(ext: SegmentationExtension, dtype: Any = None) -> bool:
    ...
    if is_fractional(ext, dtype):
        return False
    if any(v <= 0 for seg in _written(ext) for v in seg.values):
        return True
...
def _generate_from_model(
    ...
    for i, seg in enumerate(written):
        ...
        kv[f"{p}LabelValue"] = " ".join(str(v) for v in seg.sorted_values)
...
def _materialize(
    ...
    for seg in ext.segments:
        ...
        # a materialized segment lists the one value it was given, whatever it was
        update: dict[str, Any] = {"label_values": [value], "members": None,
                                  "layer": dest or None, "extent": None}
        if mask.any():
    ...
    for seg, mask, dest in zip(written, masks, destinations):
        layers_out[dest][mask] = seg.sorted_values[0]
