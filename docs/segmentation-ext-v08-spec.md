...
| sRGB | `color(srgb r g b)` — three plain numbers in 0–1, each with at most six significant digits, as C's `%.6g` formats it | the color is sRGB and is not 8-bit: a `.seg.nrrd` color set to arbitrary floats (§6.1) |
...
In the three functional forms the numbers are separated by single spaces, carry no percent signs, and are followed by no alpha component; these are also CSS's own canonical serializations. `srgb` is CSS Color Level 4's sRGB with components from 0 to 1, `lab()` is its CIELab, referenced to a D50 white, and `xyz-d65` is its CIE XYZ relative to D65; the last two can state any color exactly. Writers do not write any other CSS form — the three-digit `#rgb`, `rgb()`, an alpha component — and a reader treats a value it cannot parse as absent, and reports it. An absent color means the viewer chooses. Conversion to a display's color space is CSS Color 4's. Where a color must be expressed in sRGB — as floats for `.seg.nrrd`, at 8 bits for a color table — it is converted as CSS Color 4 defines and brought into gamut by CSS Color 4's gamut-mapping algorithm, not by clipping channels, and the change is reported when the color was out of gamut.
...
**Color.** `SegmentN_Color` is three floats in 0–1, which 3D Slicer writes with six significant digits. On import, when every channel equals `n / 255` for an integer `n`, to six significant digits — what Slicer writes for a color chosen at 8 bits — the color is written as hex. Otherwise the three numbers are transcribed as they stand: `0.5 0.333333 0.0784314` becomes `color(srgb 0.5 0.333333 0.0784314)`. On export a hex color is written as `n / 255` per channel and a `color(srgb …)` color as its three numbers, in both cases with six significant digits as C's `%.6g` formats them, so any color Slicer wrote returns byte for byte. A `lab()` or `color(xyz-d65 …)` color is converted to sRGB and brought into gamut (§3.2), and written the same way without being reduced to 8 bits.
...
2. **Simple fields.**
   ...
   | `color: [r, g, b]` floats | hex or `color(srgb …)`, chosen as in §6.1, **read as sRGB** — the first time that assumption is written down |
...
- Color needs a parser and converter for CSS Color 4, and should not be hand-written a second time. `coloraide` (Python, pure, no dependencies) and `colorjs.io` (JavaScript, by the CSS Color specification's editors) both parse the four forms and implement CSS's conversions and gamut mapping. Two things remain the implementation's own: checking that a string is one of the four *spellings* before handing it to a parser that accepts all of CSS, and formatting output to the fixed decimal places of §3.2. The D65 CIELab steps of §6.2 are five lines and are written out there so that they do not depend on a library's constants.
...
| `color-unparseable` | reader | warning | segment | `color` is not one of the four spellings and is treated as absent (§3.2) |
