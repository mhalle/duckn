...
Three forms are written, each with one spelling, and every reader accepts all three:
...
| CIELab | `lab(L a b)` — three plain numbers with exactly four decimal places | the color's source is CIELab under a D50 white (§6.2), or it is outside sRGB or finer than 8 bits |
...
In the two functional forms the numbers are separated by single spaces, carry no percent signs, and are followed by no alpha component; these are also CSS's own canonical serializations. `lab()` is CSS Color Level 4's CIELab, referenced to a D50 white, and `xyz-d65` is its CIE XYZ relative to D65; either can state any color exactly. Writers do not write any other CSS form — the three-digit `#rgb`, `rgb()`, an alpha component — and a reader treats a value it cannot parse as absent, and reports it. An absent color means the viewer chooses. Conversion to a display's color space is CSS Color 4's. Where a color must be reduced to 8-bit sRGB — for `.seg.nrrd`, for a color table — it is converted as CSS Color 4 defines and brought into gamut by CSS Color 4's gamut-mapping algorithm, not by clipping channels, and the change is reported when the color was out of gamut.

**Which color a voxel takes.** Within a layer, a voxel's recommended color is that of the **topmost segment of the layer listing its value that has a color** (§2). That is a color table with one entry per value, one table per layer. Because a segment whose values contain another's is listed first (§5), a specific structure is drawn over the general one that contains it, and a structure with no color of its own shows the color of the last containing structure before it that has one. How the layers of a layered array are composited is a viewer's choice; the recommendation is ascending layer index, higher over lower, with background voxels transparent.
...
- `dicom` — DICOM segment attributes this extension gives no meaning to, kept for a round trip: `SegmentDescription`, `TrackingID` and `TrackingUID`, and further modifier or region items as arrays of coded entries under `type_modifiers` and `anatomic_regions` (§4.2);
...
1. *error; reader refuses.* `version` is present, is a string, leads with `N.N`, and is not later than the reader supports (§3.1).
...
3. *error; reader continues.* `labeling_scheme`, when an array, has distinct entries (a reader ignores repeats). `implicit_background`, when present, is `false`, and the labelmap is binary (a reader ignores `true`, and ignores the field on a fractional labelmap).

**Identity**

4. *error.* `id` is unique across `segments` (*reader refuses*) and is a token as defined in §3.2 (*reader continues*, using the id as found).
...
8. *error; reader refuses.* `role`, when present, is `"background"` or `"unknown"`.
...
11. *error.* `label_values` is a non-empty array of integers, each representable in the array's data type and no greater in magnitude than 2^53 − 1; booleans are not integers (*reader refuses*). The entries are distinct and in ascending order (*reader continues*, reading the array as a set).
...
**Ids.** A source id that is a token is kept. The others are derived, deterministically: every source id that is already a token is reserved first; then, in segment order, each remaining id has every run of characters outside the token alphabet replaced by `_`, becomes `Segment_<N>` (its zero-based ordinal) if the result contains no letter or digit, and takes the first of the suffixes `_2`, `_3`, … that makes it differ from every reserved and every already-derived id. The original is kept under `metadata.duckn.id`. An exporter writes the original back as `SegmentN_ID` when doing so leaves all ids in the file distinct, and the token otherwise.

**Color.** On import each float channel `x` becomes the integer nearest `255 × x`, halves rounding up, and the color is written as hex. On export a color is first reduced to 8-bit sRGB (§3.2) — a hex color already is — and each channel is written as `n / 255` with six significant digits, as C's `%.6g` formats it. That is the form 3D Slicer's writer produces, so a color Slicer wrote from an 8-bit value returns byte for byte.

**Overlap on export.** 3D Slicer writes one positive `LabelValue` per segment, unique within a layer; its 0 is background and has no segment; and it represents overlap with layers. A source can be written **as it is** — segments in their layers with their values — when every layer is a label table (§5) and no segment other than a background segment lists 0. Otherwise the exporter **materializes**, which means writing new voxel data and not only new metadata:
...
**Parsing notes** (unchanged from 0.7): accept either the `Master` or `Source` representation key; normalize representation names to kebab-case; split pipe-delimited lists and drop empty elements; strip the `Segmentation.` prefix from tag keys; treat literal `\n` escape sequences in `ConversionParameters` descriptions as newlines; drop a `Layer` of 0; do not default a missing `LabelValue` to 0, assign the smallest positive integer not used in the segment's layer instead; keep a two-part coded entry whose meaning is empty.
...
- A **`LABELMAP`** is one layer whose `label_values` are the Segment Numbers. DICOM says that in this type no value is background unless `PixelPaddingValue` names it (PS3.3 C.8.20.2.4); so the imported file sets `"implicit_background": false`, and gives the background role to the segment whose number `PixelPaddingValue` names, if there is one. A segment item at value 0 is then an ordinary structure, as DICOM allows ("the Value 0, if used, will also be described, which might serve as the background," C.8.20.2.3.3). Items that are the same in every respect but their Segment Number — label, codes, algorithm — are one segment that an exporter wrote once per value (below), and are merged into one multi-valued segment whose `id` comes from the lowest number. The Zarr array needs a `fill_value` that the layer describes (rule 15): the importer uses the background value where there is one, and otherwise the smallest non-negative integer no item uses, adding a segment `{"id": "unwritten", "role": "unknown"}` that lists it. A `LABELMAP` whose photometric interpretation is `PALETTE COLOR` is read the same way; mapping its palette to `color` is not defined in this version, and its segments are left without one.
- A **`BINARY`** or **`FRACTIONAL`** object gives each segment its own frames. When `SegmentsOverlap` is `NO`, a `BINARY` object is imported as **one layer**, with the Segment Numbers as values. Otherwise each segment is placed in its own layer with `label_values: [1]`; an importer that has verified from the frames that no two segments overlap may combine them into one layer instead, and reports that it did.
...
`SegmentLabel` is required by DICOM and limited to 64 characters (not bytes): it is the segment's `name`, else the `meaning` of its first designation, else its `id`, truncated if need be and the truncation reported. `TrackingID` and `TrackingUID`, when kept in `metadata.dicom`, are written only as a pair.
...
3. **Groups are flattened, against the original ids.** A group is a 0.7 segment with `members`, or a 0.6 segment whose `label_value` has string entries, alone or mixed with integers. Its effective values are its own integers together with the values of every segment in its transitive membership. When those all lie in one layer, the group becomes a segment in **that layer** — whatever `layer` it declared itself — whose `label_values` is that set, **minus every value listed by a role-bearing segment in its transitive membership**, keeping the group's `id`, `name`, `color`, `designations`, `dicom`, and `metadata`, and dropping its `extent`. A group has no 0.8 form when its values span layers, when a member does not resolve, when the members form a cycle, or when nothing is left after the subtraction: it is omitted, its original entry is kept in the extension's `metadata.duckn.omitted` (§3.1), and the omission is reported. `disjoint` and `exhaustive` are dropped, and each discarded claim is reported. A background value subtracted from a designated group is reported, since the group may no longer cover its concept.
...
- Color needs a parser and converter for CSS Color 4, and should not be hand-written a second time. `coloraide` (Python, pure, no dependencies) and `colorjs.io` (JavaScript, by the CSS Color specification's editors) both parse the three forms and implement CSS's conversions and gamut mapping. Two things remain the implementation's own: checking that a string is one of the three *spellings* before handing it to a parser that accepts all of CSS, and formatting output to the fixed decimal places of §3.2. The D65 CIELab steps of §6.2 are five lines and are written out there so that they do not depend on a library's constants.
...
On export, this file is not a label table, so it goes to `.seg.nrrd` as two layers — 184 in one, with a new value, its layers in the other keeping theirs — and to DICOM, with classifications from the caller, as three overlapping `BINARY` segments. It *is* eligible for a `LABELMAP`, since the layer is nested: the pixel data is written unchanged, values 68 and 667 are described by their layers, and value 184 by segment 184, which is innermost for it. That item stands for the unresolved voxels only, and the exporter reports that segment 184 is only partly represented. The real CCF also has structure ids in the hundreds of millions, which no `LABELMAP` can hold (§6.2).
...
**Why `color` is a string on the segment, in three spellings.** DICOM SEG and `.seg.nrrd` each carry one recommended color per segment, so the file carries one. It is a CSS string because a triple of numbers is not a color until something says what space it is in, and no earlier version did. Hex covers what every color table holds; `lab()` and `color(xyz-d65 …)` exist so that what DICOM holds can be written down as what it is. CSS's `rgb()` was considered for sRGB colors finer than 8 bits and dropped: CSS requires implementations to keep only 8 bits of it. Each form has one spelling so that two writers produce the same string, and all three are required of every reader so that no conforming writer can hand a conforming reader a color it must discard.
...
Every report this document calls for has a code. A violation of a numbered rule is `rule-N`, about the segment, the *(layer, value)* pair, or the extension that breaks it; rule 10 has no code, since nothing can detect it. The others:

| Code | Raised by | Severity | About | Meaning |
|------|-----------|----------|-------|---------|
| `undescribed-value` | reader with data, validator | error | (layer, value) | a value in the data that nothing describes (rule 16; same as `rule-16`, named for convenience) |
| `color-unparseable` | reader | warning | segment | `color` is not one of the three spellings and is treated as absent (§3.2) |
| `color-gamut-mapped` | exporter | warning | segment | a color was out of the sRGB gamut and was mapped into it (§3.2) |
| `designation-unverified` | importer | warning | segment | a designation was taken from a source format's type code; its exactness is not established (§6.2) |
...
| `dicom-items-merged` | importer | warning | segment | identical `LABELMAP` items were merged into one multi-valued segment (§6.2) |
| `dicom-layers-combined` | importer | warning | extension | `BINARY` segments were verified disjoint and combined into one layer (§6.2) |
| `role-tag-invalid` | importer | warning | segment | a `duckn.role` tag had a value that is not a role and was left as a tag (§6.1) |
| `role-lost` | exporter | warning | segment | the target cannot express the segment's role (§6.1, §6.2) |
...
| `migrated-group-omitted` | migration | warning | segment | a group had no 0.8 form and was kept under `metadata.duckn.omitted` (§6.3) |
| `migrated-claim-dropped` | migration | warning | segment | a `disjoint` or `exhaustive` claim was discarded (§6.3) |
| `migrated-background-subtracted` | migration | warning | segment | a role-bearing member's value was removed from a flattened group (§6.3) |
| `migrated-designation-set-aside` | migration | warning | segment | a designation moved to `metadata.duckn.designations` to satisfy rule 9 (§6.3) |
| `migrated-id-changed` | migration, importer | warning | segment | an id was not a token and was replaced (§6.1, §6.3) |
