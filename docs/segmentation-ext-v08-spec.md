...
Where several segments list a value, the **topmost** is the last of them in `segments` order. Every question that needs one answer for a value — its color, its name in a tooltip, the segment a lookup returns — takes the topmost segment that can answer it (§3.2 `color`, §4.1).
...
**Version semantics.** While the major version is `0`, the *minor* version may introduce breaking changes; this overrides the duckn convention's default rule that minor increments are additive. From 1.0 onward, major increments signal breaking changes and minor increments are additive. `version` must be a string: a JSON number `0.10` is the float 0.1, and a reader that parsed it that way would mistake a later file for an earlier one. A missing or unparseable version is an error, never "older than everything." A reader for 0.8 refuses a file that declares a later version while the major version is `0`, by its version and before looking at its fields, since it cannot know what changed.
...
An open-ended object for application-specific state about the segmentation, keyed by source application or standard. Well-known keys: `slicer` holds 3D Slicer's `contained_representations`, `conversion_parameters`, and `reference_extent_offset`; `dicom` holds object-level DICOM attributes a converter wishes to keep, such as `SegmentationFractionalType`; `duckn` holds what a migration preserved (§6.3).
...
A layer with no background has no negative class: a comparison over it cannot count a false positive against the unlabeled region, and a reader should say so rather than report a score as though it could.
...
The zero-based index of the layer (position along the `list` axis) holding this segment's values; a non-negative integer. An absent `layer` means layer 0, in a layered array as in any other. When an array has more than one `list`-kind axis, `layer` indexes the first.
...
Two forms are written, each with one spelling, and every reader accepts both:

| Form | Spelling | Written when |
|------|----------|--------------|
| hex | `#rrggbb` — six lowercase hexadecimal digits | the color is sRGB at 8 bits per channel: the common case, and what `.seg.nrrd` and every color table hold |
| CIELab | `lab(L a b)` — three plain numbers with exactly four decimal places, separated by single spaces; no percentages, no alpha | any other color: one whose source is CIELab (§6.2), one outside sRGB, one finer than 8 bits |

`lab()` is CSS Color Level 4's CIELab, referenced to a D50 white, and can state any color exactly; the spelling above is also CSS's own canonical serialization. Writers do not write any other CSS form — the three-digit `#rgb`, `rgb()`, an alpha component — and a reader treats a value it cannot parse as absent. An absent color means the viewer chooses. Conversion from `lab()` to a display's color space is CSS Color 4's.

**Which color a voxel takes.** A voxel's recommended color is that of the **topmost segment listing its value that has a color** (§2). Within a layer this is a color table with one entry per value; an array with several layers has one table per layer, and how layers are stacked over one another — `segments` order is the recommendation — is not expressible in a table. A background segment is drawn beneath every other segment, whatever its position. Because a segment whose values contain another's is listed first (§5), a specific structure is drawn over the general one that contains it, and a structure with no color of its own shows the color of the nearest containing structure that has one.
...
- `dicom` — DICOM segment attributes this extension gives no meaning to, kept for a round trip: `SegmentDescription`, `TrackingID` and `TrackingUID`, `RecommendedDisplayCIELabValue` as read (§6.2), and further modifier or region items as arrays of coded entries under `type_modifiers` and `anatomic_regions` (§4.2);
- `duckn` — what a conversion or migration preserved: an original `id` that was not a token, `display`, and `designations` set aside (§6).

### 3.3 Derived Arrays

The convention keeps the metadata that describes an array when the array is derived, and drops what described its source (§4.5 of the convention). Most of `seg` describes the array's values and is kept. Three things are tied to a particular grid or a particular source file and are not:

- `extent` on every segment, and `metadata.slicer.reference_extent_offset`, are in voxel coordinates. Any operation that changes the grid or the voxels — crop, resample, reorient, edit — recomputes them or drops them.
...
Within a layer, at most one segment carries a given designation (§5); the same designation may appear in different layers. A lookup by code over a whole file therefore returns one segment per layer at most, and a consumer that needs a single one takes the topmost — the last in `segments` order — knowing that the choice between layers is its own.
...
Each rule below is marked with what a violation is:

- **error** — the file does not conform. A writer must not produce it, and a validator rejects it. A reader reports it and continues wherever this document defines a reading (it always does for rules marked *tolerated*), and otherwise refuses the file.
- **warning** — the file conforms, and something is worth a person's attention.

To **report** is to hand the caller a diagnostic — what was found, where, and how severe — *alongside* the result, and not to fail. Throughout this document, a reader, an importer, a migration, or an exporter that "reports" something returns it this way; one that "fails" or "refuses" does not produce a result. How diagnostics are typed and delivered is an implementation's business; that they reach the caller, rather than a log, is not. An implementation may offer a strict mode that turns reports into failures.
...
1. *error.* `version` is present, is a string, and leads with `N.N` (§3.1).
2. *error.* `layer`, when present, is a non-negative integer. A segment whose layer is not 0 requires a `list`-kind axis in the array, and `layer` is a valid index into it. A segment in an array with no `list` axis omits `layer`.
3. *error.* `labeling_scheme`, when an array, has distinct entries. `implicit_background`, when present, is `false`, and the labelmap is binary.

**Identity**

4. *error.* `id` is unique across `segments` and is a token as defined in §3.2.
5. *error for a writer; tolerated by a reader, which reports it.* Every `scheme` used in `designations` or `dicom` is registered in `terminologies`. Every key named by `labeling_scheme` is registered with a `uri` and a `version`.
6. *error.* When `labeling_scheme` is declared, a segment carries at most one designation in each declared scheme.
...
8. *error.* `role`, when present, is `"background"` or `"unknown"`.
9. *error.* Within a layer, no two segments carry the same designation — the same `scheme`, `code`, and `modifier`.
10. *Writer's obligation; not verifiable from the file.* A segment lists every described value of its layer whose voxels are the concept of each designation it carries (§2). A reader assumes this holds. A checker that has the scheme's hierarchy from outside the file may test the part of it the hierarchy implies — a parent's values contain its children's.

**Values**

11. *error.* `label_values` is a non-empty array of distinct integers in ascending order, each representable in the array's data type and no greater in magnitude than 2^53 − 1. Booleans are not integers.
12. *error.* Where one segment's `label_values` strictly contain another's in the same layer, the containing segment comes first in `segments`.
13. *error; binary labelmaps.* At most one segment per layer has the background role, and it lists exactly one value.
14. *error; binary labelmaps.* A layer's background value (§3.2 `role`) is listed by no segment other than its background segment. A value listed by a segment with a `role` is listed by no other segment of the layer: "nothing here" and "not evaluated" do not overlap a structure, or each other.
15. *error; binary labelmaps; checkable from metadata.* The Zarr array's `fill_value` is, in every layer, the background value or a value some segment lists. Otherwise every unwritten voxel is undescribed.
16. *error for a writer; tolerated by a reader, which reports it; binary labelmaps; requires the voxel data.* Every value present in a layer's data is the layer's background value or is listed by some segment. A reader excludes a value that is not from comparison and measurement (§2).
17. *error; fractional labelmaps.* The array has a `list` axis; every segment has a distinct `layer`; every segment's `label_values` is `[1]`.

Rules 1 and 3–14 constrain the metadata alone. Rules 2, 15 and 17 additionally need the array's shape, axes, data type, or `fill_value`. Rule 16 needs the voxel data, which this extension never requires a reader to load. Rule 10 needs knowledge the file does not contain. The convention's own rule that an axis `kind` with a required size matches `shape` applies as it does to any array.

### Two properties a reader checks

These are not constraints on files. They are conditions a reader or exporter tests to decide what it can do.

**The label table.** A layer in which every segment with no role has exactly one entry in `label_values`, and no value is shared, is a classic label table: one row per value, one color per value. Version 0.7 imposed this on every file; 0.8 makes it the special case.
...
| `SegmentN_Color` | `segments[n].color` (hex) |
...
**Ids.** A source id that is a token is kept. The others are derived, deterministically: every source id that is already a token is reserved first; then, in segment order, each remaining id has every run of characters outside the token alphabet replaced by `_`, becomes `Segment_<N>` (its zero-based ordinal) if nothing is left, and takes the first of the suffixes `_2`, `_3`, … that makes it differ from every reserved and every already-derived id. The original is kept under `metadata.duckn.id`. An exporter writes the original back as `SegmentN_ID` when doing so leaves all ids in the file distinct, and the token otherwise.

**Color.** On import each float channel `x` becomes the integer nearest `255 × x`, halves rounding up, and the color is written as hex. On export each channel is written as `n / 255` with six significant digits, as C's `%.6g` formats it — the form 3D Slicer's writer produces — so a color Slicer wrote from an 8-bit value returns byte for byte. A `lab()` color is converted to sRGB as CSS Color 4 defines, each channel is clipped to `[0, 1]`, and the clipping is reported when it changes the color.

**Overlap on export.** 3D Slicer writes one `LabelValue` per segment, unique within a layer, and represents overlap with layers. When every layer of the source is a label table (§5), nothing needs to change: segments are written in their layers with their values. Otherwise the exporter **materializes**, which means writing new voxel data and not only new metadata:

1. Segments are taken in `segments` order, role-bearing ones included, except a background segment at value 0, which is `.seg.nrrd`'s own background and is not written.
2. Each is placed in the first destination layer, counting from 0, in which it overlaps no segment already placed. Two segments from the same source layer overlap when they share a value; two from different source layers overlap when they share a voxel, which the exporter decides from the data it is rewriting.
3. In its destination layer a segment keeps its value if it has exactly one and that value is not yet used there; otherwise it takes the smallest positive integer not yet used in that layer. Its voxels are the union of its source values' voxels.

The result is deterministic given the file and its data. The voxels of every written segment are preserved; the island decomposition, the source layer assignment, and the label values are not (§7.1). A segmentation with nested value lists (§2) exports to one layer per level of nesting, which can be many full-size volumes, and an exporter reports it. `extent` is recomputed or dropped. An unmodified file with `legacy` key/values is replayed verbatim instead.
...
**Roles on export.** A role-bearing segment is written as an ordinary segment with the tag `duckn.role` set to its role. 3D Slicer keeps tags it does not recognize through a load and save, so the role survives a round trip through it; an importer takes the tag as a `role` only when its value is `background` or `unknown`, and otherwise leaves it among the tags and reports it. A segment that lists 0 cannot be written, because the format's 0 has no segment: for a background segment nothing is lost but its `name` and codes, which are reported; for an *unknown* segment at 0 the loss is one of meaning — on re-import those voxels read as background, "not evaluated" having become "nothing here" — and the exporter reports exactly that.
...
- A **`LABELMAP`** is one layer whose `label_values` are the Segment Numbers. DICOM says that in this type no value is background unless `PixelPaddingValue` names it; so the imported file sets `"implicit_background": false`, and gives the background role to the segment whose number `PixelPaddingValue` names, if there is one. A segment item at value 0 is then an ordinary structure, as DICOM means it to be. A `LABELMAP` whose photometric interpretation is `PALETTE COLOR` is read the same way, its palette supplying each value's `color` in place of `RecommendedDisplayCIELabValue`, which such an object does not have.
- A **`BINARY`** or **`FRACTIONAL`** object gives each segment its own frames. The importer places each segment in its own layer with `label_values: [1]`. It may instead combine `BINARY` segments into one layer, with the Segment Numbers as values, when `SegmentsOverlap` is `NO` or it has verified from the frames that none overlap.
...
- **`BINARY`.** Segment Numbers start at 1 and increase by 1, as DICOM requires of this type, so they number the segments written, in `segments` order. Each segment's frames are the union of its values' voxels, and segments may overlap. DICOM records only presence and absence in this type and has no background class; this extension reads absence as background, so a background segment is not written, and the loss of its `name` and codes is reported. An unknown segment is written as an ordinary segment, and the loss of its role is reported.
- **`FRACTIONAL`.** One segment per layer, numbered from 1, **role-bearing layers included**: a softmax model's background channel holds real probabilities and is written like any other, its role lost and reported. A Segmentation object has no Modality LUT, so `value_transforms` cannot be exported: the exporter requantizes the fractions into the 8 bits DICOM requires and writes the denominator it chose as `MaximumFractionalValue` (255 unless its caller says otherwise). `SegmentationFractionalType` — `PROBABILITY` or `OCCUPANCY` — is a distinction this extension does not record; the exporter takes it from the extension's `metadata.dicom` or from its caller.
- **`LABELMAP`.** A segmentation is **eligible** when it has a single layer in which, for every value, the segments listing it are nested — each one's `label_values` containing the next's. Label tables are eligible; so is a hierarchical atlas (§2); two segments that merely overlap are not. The pixel data is written **unchanged**: in this type the pixel values are the Segment Numbers, which need not start at 1 or be contiguous. DICOM requires every pixel value present to be described, so the exporter writes one segment item **per value**, described by the innermost segment listing it — the most specific structure. A segment that is innermost for no value has no item, and the exporter reports each one: a label map can hold an atlas's leaves and unresolved remainders, not its interior structures as wholes. Three conditions make the exporter fail rather than guess. Segment Number is a 16-bit unsigned attribute and the pixels are 8- or 16-bit unsigned, so every value must lie in 0–65535; an atlas with larger ids — the Allen CCF has many — must be renumbered first, by the caller's choice, which gives up the correspondence between value and id. Every value present in the data must be described (§5). And the item for a background value needs the category, type, and algorithm attributes DICOM requires of every item, which a background segment's `dicom` field or the caller must supply, since DICOM defines no code for "background"; `PixelPaddingValue` is then set to that value. A layer with no background gets no `PixelPaddingValue`. An unknown segment is written as an ordinary item, and the loss of its role is reported.

`SegmentsOverlap` is `NO` for a `LABELMAP`, as DICOM requires. For the other types an exporter that has examined the frames writes `YES` or `NO` as it found; one that has not writes `NO` when all written segments come from one layer with disjoint value lists (§5), and `UNDEFINED` otherwise.

`SegmentLabel` is required by DICOM and limited to 64 characters: it is the segment's `name`, else the `meaning` of its first designation, else its `id`, truncated if need be and the truncation reported. `TrackingID` and `TrackingUID`, when kept in `metadata.dicom`, are written only as a pair.

**Color.** `RecommendedDisplayCIELabValue` is described as being "in PCS-Values" and "encoded as CIELab" (PS3.3, the Segment Description macro), and the encoding's definition notes that "this is the same form of encoding as used for the PCS in ICC Profiles" (PS3.3 C.10.7.1.1, a note). The ICC profile connection space uses a D50 white point (stated in PS3.4 N.2.2.2, which is informative), and so does CSS Color 4's `lab()`. The 16-bit form is ICC version 4's. Read as the standard intends, then, the three values are a CSS `lab()` color with no conversion at all: `L = v × 100 / 65535`, and `a = v × 255 / 65535 − 128`, likewise `b`, written with four decimal places, from which the 16-bit integers are recovered exactly. `[39330, 30580, 41942]` is `"lab(60.0137 -9.0117 35.1984)"`.

Most files in existence were not written as the standard intends. dcmqi — and through it 3D Slicer and most public collections — computes these values from 8-bit sRGB using a D65 white and omits the chromatic adaptation to D50. Read correctly, such a value is not the color its author chose: the error is zero for grays, a few levels of 255 for muted colors, about 18 for a saturated green, and about 90 for pure blue, and it exceeds 18 levels for roughly a fifth of all sRGB colors. Read with the same D65 arithmetic, it returns the author's 8-bit color exactly. An importer therefore distinguishes the two cases:

- When the object's `Manufacturer` is `QIICR` — what dcmqi writes; its `SoftwareVersions` is a build hash and identifies nothing — the importer by default inverts dcmqi's computation, writes the author's color as hex, and keeps the three integers as read under `metadata.dicom.RecommendedDisplayCIELabValue`, which both records that this was done and lets an exporter write the original values back while `color` is unchanged.
- Otherwise it transcribes the values into `lab()` as above.

An importer's caller may override either choice. An exporter with nothing to write back converts the segment's color to `lab()` as CSS Color 4 defines and scales to 16 bits, which is what the standard asks for. When the attribute is absent the color is left absent.

### 6.3 Reading Older Files

A reader for 0.8 that accepts older files migrates on load, and reports what the migration changed (§5). The steps run in this order, which matters because several of them read what others rewrite.

1. **Pre-0.6 shapes.** 0.5 `identifiers`, the `metadata.dicom` classification, and Slicer fields become `designations`, `dicom`, and `metadata.slicer`, as 0.6 defined.
2. **Simple fields.**

   | Older shape | 0.8 shape |
   |---|---|
   | `label_value: 5` (0.7; 0.6 scalar) | `label_values: [5]` |
   | `label_value: [3, 1]` (0.6 list of integers) | `label_values: [1, 3]` |
   | `background: true` (0.7) | `role: "background"`, **reported** |
   | `color: [r, g, b]` floats | hex, each channel as in §6.1, **read as sRGB** — the first time that assumption is written down |
...
3. **Groups are flattened, against the original ids.** A group is a 0.7 segment with `members`, or a 0.6 segment whose `label_value` has string entries, alone or mixed with integers. Its effective values are its own integers together with the values of every member, resolved transitively. When those all lie in one layer, the group becomes a segment in **that layer** whose `label_values` is that set, **minus any value listed by a role-bearing member**, keeping the group's `id`, `name`, `color`, `designations`, `dicom`, and `metadata`, and dropping its `extent`. When they span layers, or a member does not resolve, or the members form a cycle, the group has no 0.8 form: it is omitted, its original entry is kept under the extension's `metadata.duckn.omitted`, and the omission is reported. `disjoint` and `exhaustive` are dropped, and each discarded claim is reported. A background value subtracted from a designated group is reported, since the group may no longer cover its concept.
4. **Designation collisions are resolved.** Where a migrated group and another segment of its layer carry the same designation, the other segment was the group's unresolved remainder and its designation is no longer exact (§2). That designation moves to the segment's `metadata.duckn.designations`, and the move is reported. The result satisfies rule 9.
...
6. **Order.** Segments are reordered only as far as rule 12 requires: a segment is moved to just before the first segment whose values it strictly contains. 0.7 resolved the color of an uncolored leaf from the *first* group in document order that contained it; 0.8 takes the *topmost* containing segment that has a color, which after this step is the most specific one. The two agree unless a 0.7 file listed a broader colored group before a narrower colored one; the migration reports each value whose color resolves differently.
...
- Colors that a pre-0.8 DICOM import computed from CIELab were read with a D65 white (§6.2). For the files dcmqi wrote, which is most of them, that reading recovered the author's color, and the migrated hex is right. For a file written as the standard intends it was not, and the original values are not recoverable from a 0.7 file.
...
- A value no longer resolves to one segment. Lookups by value — `leaf_for`, `SegAccessor.segment(label_value=…)`, `name_for` — return every segment listing the value, with a variant returning the topmost. `label_for` returns a list of values.
...
- Validation distinguishes errors from warnings and returns diagnostics (§5); migration returns them too, which means it cannot live inside a model validator that has nowhere to put them.
...
Renditions, groupings, inexact correspondences, translations, and measurements live in documents outside the file. For those documents to be possible, this extension guarantees the following. How a reference is *spelled* — how an external document names an array and a segment within it — is that document's specification's business, not this one's; the convention's transform specification already refers to arrays with a structured object carrying a `path`, and that is the likely model.
...
The layer is a label table (§5). The three designations of each kidney are co-extensive, which is what lets them share a segment; TA2 has no laterality, and the cross-scheme modifier makes its designation exact all the same. The file goes to `.seg.nrrd` with its values unchanged, and to DICOM as `BINARY` with Segment Numbers 1 and 2, or as a `LABELMAP` keeping 2 and 3 if the caller asks for one and supplies the left kidney's classification and a description of value 0.
...
On export, this file is not a label table, so it goes to `.seg.nrrd` as two layers — 184 in one, its layers in the other — and to DICOM as three overlapping `BINARY` segments. It *is* eligible for a `LABELMAP`, since the segments listing each value are nested: the pixel data is written unchanged, values 68 and 667 are described by their layers, value 184 by segment 184 — which in that object stands for the unresolved voxels only — and the exporter reports that the frontal pole as a whole has no item. The real CCF also has structure ids in the hundreds of millions, which no `LABELMAP` can hold (§6.2).
...
**Why the topmost segment answers.** Once a value can belong to several segments, every convenience that used to return "the" segment for a value needs a rule, and there should be one rule. If a color table took the last segment and a lookup took the first, a viewer would paint the overlap red and call it "Liver." So the last segment in `segments` order that can answer is the answer, for color, for name, and for lookup alike, and the ordering rule makes that the most specific structure.
...
**Why "report" is defined.** A format that is strict on write and tolerant on read produces, at every tolerance, something a person should know. If that goes to a log it is lost, and if it is an exception the tolerance is gone. So a report is a diagnostic returned beside the result, every rule says whether breaking it is an error or a warning and whether a reader has a defined way to carry on, and an implementation that wants to be strict can turn reports into failures.

**Why `color` is a string on the segment, in two spellings.** DICOM SEG and `.seg.nrrd` each carry one recommended color per segment, so the file carries one. It is a CSS string because a triple of numbers is not a color until something says what space it is in, and no earlier version did. Hex covers what every color table holds. `lab()` covers everything else, exactly, and is DICOM's own encoding read as the standard intends. CSS's `rgb()` was considered for sRGB colors finer than 8 bits and dropped: CSS requires implementations to keep only 8 bits of it, so it does not deliver the precision it would be there for. Each form has one spelling so that two writers produce the same string, and both forms are required of every reader so that no conforming writer can hand a conforming reader a color it must discard.

**Why dcmqi's colors are inverted by default.** The standard means D50, and nearly every segmentation object in existence was written by software that assumed D65 and skipped the adaptation. Transcribing those numbers faithfully would display almost every real file in colors its author never chose — by as much as a third of the range — where the released importer, which makes the same mistake in reverse, displays them exactly. Fidelity to the author beats fidelity to the bytes here, provided the bytes are kept: the original values are preserved beside the color, which makes the correction visible and the round trip exact. The trigger is `Manufacturer`, which dcmqi sets to a fixed string; its software version is a build hash and identifies nothing.
...
**Why ids are tokens, and why references are not spelled here.** An id is the one part of a file that other documents embed. Stating the alphabet positively costs nothing for ids as they usually occur, and moves free text to `name`, where it was always meant to be. An earlier draft went further and defined a reference syntax, with dots and a `#`; but the convention already refers to arrays with a structured object in its transform specification, and how an external document names its targets is that document's design. The file's part is to make any spelling safe, which a token does. The price is paid at the edges: a converter derives a token from a source id that is not one, and an id cannot survive a format that has nowhere to keep it.
