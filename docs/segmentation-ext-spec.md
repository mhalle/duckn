...
A change of data type is such an operation when it changes a value: a cast to a type that cannot hold every listed value, which clamps or wraps them, must not carry `seg` forward, and neither must a cast of fractions to integers. And because the data type alone may decide whether a labelmap is binary or fractional (§3.1), an operation that moves a segmentation between integer and floating-point types writes `source_representation` when it was absent, with the reading the source array had.
...
9. *error; reader continues.* Within a layer, no two segments carry the same designation (§4.1), and no segment carries the same designation twice (a reader ignores the repeat). A reader's lookup by that designation returns the topmost of them.
...
**Why `members` is a spelling, and why the claims left.** Two of the three things 0.7's groups did are gone. Overlap is handled by shared values. And a partition claim — these classes are mutually exclusive and exhaustive — is a fact that needs no group: the segments of a label-table layer are disjoint by construction, and a softmax layer is a partition of its classes by construction. The third, a union stated once, stays, because the alternative is the Allen CCF written as a thousand overlapping integer lists. But it stays as a *spelling* of a value set, which is why no rule or lookup after §3.2 speaks of it: a reader resolves membership on load and thereafter holds effective value sets, so every rule and lookup is defined once, on values. That is also what keeps 0.7's troubles out. A group that lists integers and members, a member in another layer, a background member, a cycle — each was a way for a group's voxels to mean two things, and each is now a refusal. What a file may *not* do is invent a union: a composite belongs in the file when a scheme or a coding system defines it, so that it can carry a code. The tree of a scheme still belongs to the scheme, and a file that names the scheme and version has said which graph applies; a file's own `members` state the unions its writer had the authority to state, and nothing more. One thing is genuinely lost: a structure whose voxels span layers has no 0.8 form.
...
| `rule-2`, `rule-4b`, `rule-6`, `rule-7`, `rule-8a`, `rule-8b`, `rule-11a`, `rule-11b`, `rule-11c` | the segment that breaks the rule |
