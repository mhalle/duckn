...
**Status:** Early draft, **parked**. Not implemented, and not a dependency of the `seg` 0.8 draft, which is being finished first.

> **Open issues recorded from review, to be resolved when this draft is picked up again.** (1) `windows` has two incompatible types — a list of windows at the array level, a list of lists-or-null on an axis; the per-axis field needs its own name. (2) Per-slice windows already have a home the released importer writes and the JS readers read, `axes[].samples[].metadata.dicom`; precedence between the two is unstated. (3) Whether a `list` axis counts as "more than one component per sample" is undefined, which decides whether per-echo windows and windows on fractional segmentations are allowed. (4) RGB DICOM images may carry window attributes; the mapping table and rule 5 disagree about them. (5) A DICOM window narrower than 1 (ADC, some PET) degenerates to a step under `linear`; a converter should probably write `linear-exact` and say so. (6) Whether a `null` per-sample entry falls back to the array-level window. (7) The "(middle slice)" name suffix is not idempotent across a round trip. (8) The released resampler clears `axes[].samples` but leaves per-axis extensions, so per-sample windows would survive a resample that §5 says drops them. (9) The convention's notes still say window and level do not belong in it; §5 asks only for the derived-arrays change. (10) No severity or reporting model for the rules and the "reports" in §4. From the second standards review: (11) PET permits `MONOCHROME2` only, so `invert` cannot be exported to it, and Enhanced CT/MR likewise; and `PresentationLUTShape` *is* an attribute of CT and MR images (Type 3, General Image Module), so §4.1's export sentence is wrong on both counts. (12) `VOILUTFunction` is single-valued per image: all windows of an image share one function, an unrecognized value drops every window, and mixed functions cannot be exported to a classic image. (13) The Frame VOI LUT macro limits the sequence to one item, not the window attributes to one value. (14) The middle-instance rule needs `floor(n/2)` stated; Slicer indexes its database file list rather than geometric order, and only Slicer and Weasis keep all of the instance's windows. (15) The `cal_max > cal_min` rule matches the NIfTI reference library rather than the header text; NIfTI-2 and `sigmoid` export are unaddressed. (16) §5 should say *convex combination* (non-negative weights summing to one) — an unsharp mask sums to one and must drop the window — and a clipping cast belongs on the dropped side; the base spec's §4.4 is about intercepts, not units.
