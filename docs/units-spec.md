...
When structured unit objects are used, the top-level `"duckn"` object may include a `unit_systems` field that registers the unit systems referenced in the file. This follows the same pattern as the seg extension's `terminologies` registry.
